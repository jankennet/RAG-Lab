...
export default function ChatView({ chatId }: { chatId: string }) {
  ...
  const suggestionPrompts = [
    ...
    "What can this document tell me about...",
    "Compare two attached files",
    "Draft a SQL query against this data",
  ];
  ...
  const handleProviderChange = useCallback((provider: LlmProvider) => setProvider(provider), [setProvider]);
  const handleModelChange = useCallback((model: string) => setModel(model), [setModel]);
            ...
            <div className="flex flex-col items-center justify-center text-center pt-12 pb-6 space-y-6">
              <div className="space-y-2">
                <h1 className="text-3xl font-semibold text-text tracking-tight">
                  Hello{thread.title && thread.title !== "New chat" ? `, ready when you are` : ""}.
               </h1>
                <p className="text-sm text-muted max-w-md mx-auto">
                  {currentAttachments.length > 0
                    ...
                    : thread.scope === "chat"
                      ...
                      : `Scoped to ${scopeLabel(thread.scope).toLowerCase()} — ask a question or attach a document to enrich it.`}
               </p>
             </div>
